...
bool Filled = false;
	...
	if (activeAttPlans > 0)
    {
        for (i = 0; < activeAttPlans)
        {
		   ...
		   int numTransportPlan = aiPlanGetNumberUnits(TransportAttPlanID, transportPUID);
           ...
           (planState == cPlanStateGather) && (xsGetTime() > attackPlanStartTime + 5*60*1000) || (planState == cPlanStateEnter) && (xsGetTime() > attackPlanStartTime + 5*60*1000)
