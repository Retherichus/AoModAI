    ...
    if (activeDefPlans > 0)
    {
        ...
        for (i = 0; < activeDefPlans)
        {
            ...
            else if (defendPlanID == gSettlementPosDefPlanID)
            {
                ...
                NumEnemyBuildings = NumEnemyBuildings - NumEnemyFarms; 	
    ...
    inactive
{
    ...
    if (activeDefPlans > 0)
    {
        for (i = 0; < activeDefPlans)
        {
            ...
            {
			int Agg = findStdPlan("AutoGPFoodHuntAggressive",cResourceFood, cPlanGather);
    ...
    if (activeAttPlans > 0)
    {
        for (i = 0; < activeAttPlans)
        {
            ...
            if (attackPlanID == gEnemySettlementAttPlanID)
            {
				...
				if ((aiPlanGetState(attackPlanID) == cPlanStateTransport) && (numTransport < 1) && (gTransportMap == true))
                    {
                       aiPlanDestroy(attackPlanID);
                       gEnemySettlementAttPlanTargetUnitID = -1;
                       if (ShowAiEcho == true) aiEcho ("destroying gEnemySettlementAttPlanID as it got stuck in a transport loop");
					   return;
                    }
		...
		if ((attackPlanID == gLandAttackPlanID) && (kbGetAge() == cAge2)) // allow the plan to go off in Classical without siege
	    Converted = true;
        }
    }
  ...
  // StandardAI Pop
   ...
   //
    ...
    if (targetSettlementCloseToMB == false)
    {
        ...
        else if ((numSiegeWeapons < 1+aiRandInt(2)) && (aiRandInt(10) > 2) && (Converted == false))
        {
    ...
    inactive
{
    ...
    if (activeAttPlans > 0)
    {
        for (i = 0; < activeAttPlans)
        {
            ...
            if (attackPlanID == gLandAttackPlanID)
            {
                ...
                {
                    aiPlanDestroy(attackPlanID);
                    if (ShowAiEcho == true) aiEcho("destroying gLandAttackPlanID as it got stuck in a transport loop");
                    return;
                }             
                ...
                if ((aiPlanGetState(attackPlanID) < cPlanStateAttack)
                 && (((xsGetTime() > attackPlanStartTime + 5*60*1000) && (attackPlanStartTime != -1))
                  || (aiPlanGetVariableInt(attackPlanID, cAttackPlanNumberAttacks, 0) > 2)))
                {
                    aiPlanDestroy(attackPlanID);
    ...
    {
        if ((numSiegeWeapons < 1) && (numTargetPlayerSettlements < 1) && (aiRandInt(6) < 1))
        {
            if (ShowAiEcho == true) aiEcho("returning as numSiegeWeapons < 1 and RNG said it was bad" );
            return;
        }

        else if ((foodSupply < 50) || (goldSupply < 50) && (UseStandardPop == false) && (currentPop <= currentPopCap - 3 - number)
