...
{   
	...
	{   
		...
		if (desiredPlans < numPlans)
		desiredPlans = numPlans;    // Try to preserve existing plans
	    if ((i == 1) && (numberMainBaseSites > 1) && (ResBaseEmpty == false))
...
inactive
{
	...
	if ((kbGetAge() > cAge1) && (xsGetTime() > 12*60*1000))
	distance = 50;	
	...
	if (MoreFarms >= 20)
	MoreFarms = 20;
    if ((cMyCulture == cCultureAtlantean) && (MoreFarms >= 7))
	MoreFarms = 7;
...
inactive
{
    ...
    int numAlliedSettlementsInR100 = getNumUnitsByRel(cUnitTypeAbstractSettlement, cUnitStateAlive, -1, cPlayerRelationAlly, tradeMarketPosition, 250);
    if (numAlliedSettlementsInR100 > 0)
    {
        ...
        {
            int alliedSettlementIDInR100 = findUnitByRelByIndex(cUnitTypeAbstractSettlement, i, cUnitStateAlive, -1, cPlayerRelationAlly, tradeMarketPosition, 250);
			...
			if (action == -1)   //check the currentTradeRouteLength of all tradeUnits
			{
				...
				if (targetID != -1)
				{
					...
					if (currentTradeRouteLength > minRequiredDistance)
					{
						...
						if ((tradeRouteLength > currentTradeRouteLength) && (tradeRouteLength <= 100.0))
						{
