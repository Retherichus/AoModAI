    ...
    group Sunwukong
{
    ...
    if ((kbGetAge() < cAge3 && woodSupply < 650) || (kbUnitCount(cMyID, cUnitTypeMonkeyKing, cUnitStateAlive) < 2))
	return;
    ...
    group Huangdi
{
    ...
    if ((kbGetAge() < cAge3 && woodSupply < 650) || (kbUnitCount(cMyID, cUnitTypeTerracottaSoldier, cUnitStateAlive) < 2))
	return;
...
{
    float goldSupply = kbResourceGet(cResourceGold);
    if ((gAgeFaster == true && kbGetAge() < AgeFasterStop) || (goldSupply < 1000) || (kbUnitCount(cMyID, cUnitTypeJiangshi, cUnitStateAlive) < 2))
        return;
