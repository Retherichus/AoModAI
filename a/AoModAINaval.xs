    ...
    inactive
{
	...
	TotalBuilders = TotalBuilders * 2.5;
	
    int ActivePlans = findPlanByString("Remote Settlement Transport", cPlanTransport, -1, true);
	if ((ActivePlans >= 1) || (TotalBuilders < 14) || (kbCanAffordUnit(cUnitTypeSettlementLevel1, cEconomyEscrowID) == false) || (xsGetTime() < PlanStartTime + 2.5*60*1000))
	{
	if (ActivePlans >= 1)
