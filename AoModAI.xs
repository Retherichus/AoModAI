...
extern int gtrainTradeUnitPlanID = -1;
//New globals end
...
extern int sVinlandMG = -1;
extern bool VinOkToChange = false;
...
active
{
    ...
    for (res = 0; < cNumResourceTypes+1)
    {
		...
		{
		    //Doesn't add anything, it just checks and corrects the values.. kinda
		    AddResourceToEscrow(res, cEconomyEscrowID, 0);
		    AddResourceToEscrow(res, cMilitaryEscrowID, 0);
		}
	}
	...
	int RootG = kbEscrowGetAmount(cRootEscrowID, cResourceGold);
	int RootW = kbEscrowGetAmount(cRootEscrowID, cResourceWood);
    ...
    if ((kbGetAge() > cAge1) && (kbUnitCount(cMyID, cUnitTypeAbstractVillager, cUnitStateAliveOrBuilding) < VilPop*0.80))
    {   
        
        if (RootAndEcoF < ReserveAmount)
        {
            AddResourceToEscrow(cResourceFood, cEconomyEscrowID, ReserveAmount-RootAndEcoF);
        ...
        {
	        AddResourceToEscrow(cResourceWood, cEconomyEscrowID, ReserveAmount-RootAndEcoW);
...
float econGoldEscrow=0.5, float econFavorEscrow=0.5)
{
	...
	if ((gtrainTradeUnitPlanID != -1) && (vilPop >= 0.90*econPop) || (gGlutRatio >= 1.15))
    aiPlanSetVariableInt(gCivPopPlanID, cTrainPlanFrequency, 0, 14);
	else
	aiPlanSetVariableInt(gCivPopPlanID, cTrainPlanFrequency, 0, 1);	
...
active
{
	...
	float econFoodEscrow = 1.0;	    
	float econGoldEscrow = 1.0;			
    float econWoodEscrow = 1.0;
	updateGlutRatio();
	...
	else
	{
		...
		civPopTarget = 35;
	    civPopTarget = xsMin(xsMax(civPopTarget, 20), 80);		
    ...
    if ((kbGetAge() > cAge1) || (kbGetAge() == cAge1) && (AgingUp() == true))
	{
	    ...
	    econAdjust = xsMin(xsMax(econAdjust, -1.0), 1.0);
		
		econPercent = adjustSigmoid(econPercent, econAdjust, 0.0, 1.0);
		econEscrow = econPercent;
	    econEscrow = xsMin(xsMax(econEscrow, 0.15), 0.70);
		econWoodEscrow = econEscrow;
	    econFoodEscrow = econEscrow;
		econGoldEscrow = econEscrow;
	    		
		if ((xsGetTime() > 10*60*1000) && (kbGetAge() == cAge2) && (AgingUp() == false) || 
		(AgingUp() == false) && (xsGetTime() - ageStartTime3 > 5*60*1000) && (kbGetAge() == cAge3)
		...
		{
		    float goldSupply = kbEscrowGetAmount(cEconomyEscrowID, cResourceGold);
            float woodSupply = kbEscrowGetAmount(cEconomyEscrowID, cResourceWood);
            float foodSupply = kbEscrowGetAmount(cEconomyEscrowID, cResourceFood);
		    if ((kbGetAge() == cAge4) && (woodSupply < 800))
			{
		        if (econWoodEscrow < 0.50)
			    econWoodEscrow = 0.50;
				int MilWoodSupply = kbEscrowGetAmount(cMilitaryEscrowID, cResourceWood)+kbEscrowGetAmount(cRootEscrowID, cResourceWood);
				if (MilWoodSupply > 100)
				AddResourceToEscrow(cResourceWood, cEconomyEscrowID, MilWoodSupply-100);					
			}
			
			if ((kbGetAge() == cAge2) && (foodSupply < 800) 
			|| (kbGetAge() == cAge3) && (foodSupply < 1000)
			|| (kbGetAge() == cAge4) && (foodSupply < 800))
			{
			    if (econFoodEscrow < 0.50)
	            econFoodEscrow = 0.50;
				int MilFoodSupply = kbEscrowGetAmount(cMilitaryEscrowID, cResourceFood)+kbEscrowGetAmount(cRootEscrowID, cResourceFood);
				if (MilFoodSupply > 100)
				AddResourceToEscrow(cResourceFood, cEconomyEscrowID, MilFoodSupply-100);					
			}
			if ((kbGetAge() == cAge2) && (goldSupply < 500) 
			|| (kbGetAge() == cAge3) && (goldSupply < 1000)
	        || (kbGetAge() == cAge4) && (goldSupply < 800))
			{
			    if (econGoldEscrow < 0.50)
		        econGoldEscrow = 0.50;
			    int MilGoldSupply = kbEscrowGetAmount(cMilitaryEscrowID, cResourceGold)+kbEscrowGetAmount(cRootEscrowID, cResourceGold);
			    if (MilGoldSupply > 100)
			    AddResourceToEscrow(cResourceGold, cEconomyEscrowID, MilGoldSupply-100);
			}
		}	
	...
	updateEM(civPopTarget, milPopTarget, econPercent, 0.20, econFoodEscrow, econWoodEscrow, econGoldEscrow, econEscrow);
}
...
active
{
    ...
    if (kbUnitCount(cMyID, cUnitTypeMarket, cUnitStateAlive) > 0)
    {
		...
		if (kbResourceGet(cResourceGold) > 1800)	// We have a lot of gold, OK to buy
        {
            if (kbResourceGet(cResourceFood) < 800)
            {
                aiBuyResourceOnMarket(cResourceFood);
        ...
        if (kbResourceGet(cResourceFood) > 2000)	// We have a lot of food, OK to sell
        {
            if (kbResourceGet(cResourceGold) < 800)
            {
                aiSellResourceOnMarket(cResourceFood);
			    xsSetRuleMinIntervalSelf(2);
			}
        ...
        if (kbResourceGet(cResourceWood) > 1800)	// We have a lot of food, OK to sell
        {
            if (kbResourceGet(cResourceGold) < 800)
            {
                aiSellResourceOnMarket(cResourceWood);
		...
		{
			if (gFoodGlutRatio < 1.10)
			{
				aiBuyResourceOnMarket(cResourceFood);
				xsSetRuleMinIntervalSelf(2);
			}
			if (gWoodGlutRatio < 1.10)
			{
				aiBuyResourceOnMarket(cResourceWood);
				xsSetRuleMinIntervalSelf(2);
			}		
...
void updateGathererRatios(void) //Check the forecast variables, check inventory, set assignments
{
    ...
    if (kbGetAge() > cAge1)
	{
        int minFoodGatherers = 6 + numGatherers*0.20;
	    if (aiGetWorldDifficulty() == cDifficultyEasy)
	...
	{
        if ((kbGetAge() == cAge4) && (TitanAvailable == true) && (kbGetTechStatus(cTechSecretsoftheTitans) < cTechStatusResearching) && (favorSupply <= 50))
        aiSetResourceGathererPercentage(cResourceFavor, 0.12, false, cRGPScript);	
...
void addUnitForecast(int unitTypeID=-1, float qty=1)
{
	if (unitTypeID < 0)
	return;
...
void AddGenForecastNeeds(void)
{
	...
	{	
        if (gBuildWalls == true)
		gGoldForecast = gGoldForecast + 150;
		...
		addUnitForecast(cUnitTypeSettlementLevel1, 2);
	    else 
		...
		if (kbGetAge() >= cAge3)
		{
			...
			// Fortress, etc.
            addUnitForecast(MyFortress, 1.5);
			
		    // Fortified TC
		    if (kbGetTechStatus(cTechFortifyTownCenter) < cTechStatusResearching) 
		    addTechForecast(cTechFortifyTownCenter);
			// Trade carts
			if (gtrainTradeUnitPlanID != -1)
			{   // We're making trade carts
				int cartsNeeded = gMaxTradeCarts - kbUnitCount(cMyID, cUnitTypeAbstractTradeUnit, cUnitStateAlive);
				if (cartsNeeded > 6)
				cartsNeeded = 6;
				if (cartsNeeded > 0)
				addUnitForecast(cUnitTypeAbstractTradeUnit, cartsNeeded);
			}
		}
	}		
}
...
{	
    ...
    if ((ageStartTime != -1) && (xsGetTime() - ageStartTime > 2*60*1000))
    {
        if (foodSupply < 1400)
...
inactive
{
    ...
    if ((ageStartTime != -1) && (xsGetTime() - ageStartTime > 2*60*1000))
    {
        ...
        if (foodSupply < 1800)
        gFoodForecast = gFoodForecast + (1800 - foodSupply);
        if (woodSupply < 500)
        gWoodForecast = gWoodForecast + (500 - woodSupply);
    }	
    else
    {
        ...
        if (foodSupply < 400)
        gFoodForecast = gFoodForecast + (400 - foodSupply);
        if (woodSupply < 300)
	...
	gWoodForecast = gWoodForecast * 0.75;
...
inactive
{
    ...
    if ((ageStartTime != -1) && (xsGetTime() - ageStartTime > 2*60*1000) || (ShouldIAgeUp() == true))
    {
        if (goldSupply < 800)
...
inactive
{
    ...
    addUnitForecast(cUnitTypeSettlementLevel1, 1.4);
...
void initAtlantean(void)
{
	...
	aiPlanSetEscrowID(oracleMaintainPlanID, cRootEscrowID);
...
int maxUnits=20, int minPop=-1, int maxPop=-1, int numberBuildings=1)
{
    ...
    kbUnitPickSetAttackUnitType(upID, cUnitTypeHumanSoldier);
    kbUnitPickSetGoalCombatEfficiencyType(upID, cUnitTypeHumanSoldier);
	...
	{	
		case cCultureGreek:
		{
			kbUnitPickSetPreferenceFactor(upID, cUnitTypePeltast, 0.3);
			...
			if (cMyCiv == cCivHades)
			kbUnitPickSetPreferenceFactor(upID, cUnitTypeCrossbowman, 0.5);	
		    kbUnitPickSetPreferenceFactor(upID, cUnitTypePhysician, 0.0);
			break;
		}
		case cCultureEgyptian:
		{
			...
			kbUnitPickSetPreferenceFactor(upID, cUnitTypeWarElephant, 0.6);
		...
		case cCultureAtlantean:
		{
			...
			kbUnitPickSetPreferenceFactor(upID, cUnitTypeAbstractCavalry, 0.35);
			kbUnitPickSetPreferenceFactor(upID, cUnitTypeMaceman, 0.15);
			break;
		}			
...
void init(void)
{
   ...
   if (rushSize > 50)
    ...
    if (gGatherGoalPlanID >= 0)
    {
        ...
        updateEMAllAges();
		kbEscrowSetCap(cEconomyEscrowID, cResourceFood, 200.0);
		kbEscrowSetCap(cEconomyEscrowID, cResourceWood, 100.0);     
		kbEscrowSetCap(cEconomyEscrowID, cResourceGold, 100.0);    		
		kbEscrowAllocateCurrentResources();
	}
...
void age2Handler(int age=1)
{
    ...
    else if (cMyCulture == cCultureNorse)
    {
        ...
        gHero1MaintainPlan = createSimpleMaintainPlan(cUnitTypeHeroNorse, 1, false, kbBaseGetMainID(cMyID));
        aiPlanSetDesiredPriority(gHero1MaintainPlan, 20);
