...
extern float gMainBaseAreaWallRadius = 30;
    ...
    active
{
    ...
    if (kbGetAge() > cAge1)
    {
        if (foodSupply < ReserveAmount)
        {
			...
			if ((cMyCulture == cCultureAtlantean) && (woodSupply < 40))
...
void updateGathererRatios(void) //Check the forecast variables, check inventory, set assignments
{
	...
	if ((ResInflate == true) && (foodSupply > 2200))
	foodMultiplier = 1.0;
    ...
    if (neededFoodGatherers > 0)
    {
        ...
        {
            foodAssignment = lastFoodAssignment + 0.05;
        }
