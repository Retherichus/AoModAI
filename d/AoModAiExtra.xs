...
void initRethlAge2(void)
{
	...
	vector HomeBase = kbBaseGetLocation(cMyID, kbBaseGetMainID(cMyID));
	 aiUnitCreateCheat(cMyID, cUnitTypeFlyingMedic, HomeBase, "Spawn backup roc", 1);
	  aiUnitCreateCheat(cMyID, cUnitTypeStymphalianBird, HomeBase, "Spawn backup roc", 1);
	   aiUnitCreateCheat(cMyID, cUnitTypePhoenix, HomeBase, "Spawn backup roc", 1);
	    aiUnitCreateCheat(cMyID, cUnitTypeRoc, HomeBase, "Spawn backup roc", 1);
		aiUnitCreateCheat(cMyID, cUnitTypeVermilionBird, HomeBase, "Spawn backup roc", 1);
		aiUnitCreateCheat(cMyID, cUnitTypeWarSalamander, HomeBase, "Spawn backup roc", 4);
   ...
   active
{
    if (kbGetAge() > cAge2)
    {
		...
		if (gTransportMap == true)
		xsEnableRule("ForbiddenTransportUnits");
		}
...
inactive
{
   ...
   int ForbiddenUnit = -1;
    ...
    switch (cMyCulture)
    {
        ...
        {
		   // if ((kbUnitCount(cMyID, cUnitTypeRoc) > 0) || (cMyCiv == cCivRa))
           // ForbiddenUnit = cUnitTypeRoc;
			//else
			ForbiddenUnit = cUnitTypePhoenix;
            break;
        }
        case cCultureNorse:
        {
        ...
        {
		    if ((cMyCiv == cCivGaia) || (cMyCiv == cCivOuranos))
            ForbiddenUnit = cUnitTypeStymphalianBird;
			else 
			ForbiddenUnit = -1;
            break;
        }
        ...
        {
		    if ((cMyCiv == cCivNuwa) || (cMyCiv == cCivFuxi))
            ForbiddenUnit = cUnitTypeVermilionBird;
			else 
			ForbiddenUnit = -1;			
            break;
        }		
    }
    //aiEcho("My unit is:" +kbGetProtoUnitName(ForbiddenUnit));
	if ((ForbiddenUnit == -1) || (gTransportMap == false)) // disable if invalid!
    {
      xsDisableSelf();
	  //aiEcho("Error, this civ doesn't have a valid unit!");
	  return;
    }
   
   int NumForbidden = kbUnitCount(cMyID, ForbiddenUnit);
   int transportPUID=kbTechTreeGetUnitIDTypeByFunctionIndex(cUnitFunctionWaterTransport, 0);
   if ((transportPUID < 0) || ((NumForbidden < 1)))
   return;
   ...
   {
		kbUnitQuerySetPlayerID(unitQueryID, cMyID);
	    kbUnitQuerySetUnitType(unitQueryID, transportPUID);
	    kbUnitQuerySetState(unitQueryID, cUnitStateAlive);
	    kbUnitQuerySetAscendingSort(unitQueryID, false);
	    kbUnitQuerySetMaximumDistance(unitQueryID, 0);
   }
   ...
   {
		kbUnitQuerySetPlayerRelation(enemyQueryID, cPlayerRelationSelf);
		kbUnitQuerySetUnitType(enemyQueryID, ForbiddenUnit);
	    kbUnitQuerySetState(enemyQueryID, cUnitStateAlive);
		kbUnitQuerySetSeeableOnly(enemyQueryID, true);
		kbUnitQuerySetAscendingSort(enemyQueryID, true);
		kbUnitQuerySetMaximumDistance(enemyQueryID, 10);
   ...
   for (i=0; < siegeFound)
   {
	   ...
	   {
		enemyUnitIDTemp = kbUnitQueryGetResult(enemyQueryID, 0);
		aiTaskUnitDelete(enemyUnitIDTemp); // delete the fucker
	   }
   }
}
