    ...
    if (farmers > 0)
    {
        ...
        if (farmersAtMainBase > 0)
        {
            ...
            else
            {
                ...
                numFarmPlansWanted = 1 + (farmers / 20);
				if (numFarmPlansWanted < 2)
				{
				aiPlanSetVariableInt(gGatherGoalPlanID, cGatherGoalPlanFarmLimitPerPlan, 0, 20);
				aiSetFarmLimit(20);
				}
				int numFarmsNearMainBaseInR70 = getNumUnits(cUnitTypeFarm, cUnitStateAlive, -1, cMyID, mainBaseLocation, 70.0);
				if ((numFarmPlansWanted >= 2) || (numFarmsNearMainBaseInR70 >= 20))
				{
