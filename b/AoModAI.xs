...
extern int gEarlySettlementTarget = 2;       // How many age 1/2 settlements do we want?
...
extern float gMainBaseAreaWallRadius = 26;
    ...
    active
{
    ...
    static int count = 0;
	static int countB = 0;
	int Dropsite = cUnitTypeEarlyFoodDropsite;
	int ReserveAmount = 150;
	if (cMyCulture == cCultureAtlantean)
	{
	Dropsite = cUnitTypeGuild;
	ReserveAmount = 125;
    }
    ...
    if (kbGetAge() > cAge1)
    {
        if ((foodSupply < ReserveAmount) || (cMyCulture == cCultureAtlantean) && (kbCanAffordUnit(kbTechTreeGetUnitIDTypeByFunctionIndex(cUnitFunctionGatherer,0), cEconomyEscrowID) == false))
        {
		    if (foodSupply < ReserveAmount) // atlanteans need special treatment
            kbEscrowFlush(cMilitaryEscrowID, cResourceFood, true);
            if (ShowAiEcho == true) aiEcho("Flushing military food escrow");
			if ((cMyCulture == cCultureAtlantean) && (woodSupply < 50))
            ...
            {
                if ((aiGetMarketBuyCost(cResourceFood) < goldSupply) && (count > 0) && (foodSupply < 90))
                {
            ...
            if ((kbUnitCount(cMyID, cUnitTypeMarket, cUnitStateAlive) > 0) && (woodSupply < 25) (cMyCulture == cCultureAtlantean) && (kbEscrowGetAmount(cEconomyEscrowID, cResourceWood) < 25))
            {
                if ((aiGetMarketBuyCost(cResourceWood) < goldSupply) && (countB > 0) && (woodSupply < 25))
                {
    ...
    if (kbGetAge() == cAge2)
    {
        ...
        if ((kbGetTechStatus(cTechPlow) < cTechStatusResearching) && (aiPlanGetIDByTypeAndVariableType(cPlanProgression, cProgressionPlanGoalTechID, cTechPlow, true) >= 0) && (numFarms >= 3))
        {
            ...
            {
                aiTaskUnitResearch(findUnit(Dropsite), cTechPlow);
		...
		(kbUnitCount(cMyID, cUnitTypeAbstractSettlement, cUnitStateAliveOrBuilding) < gEarlySettlementTarget))
        {
            if ((cMyCulture != cCultureEgyptian) && (woodSupply > 350) && (goldSupply > 350) && (foodSupply < 560) || (cMyCulture == cCultureEgyptian) && (goldSupply > 400) && (foodSupply < 560))
            {
    ...
    else if (kbGetAge() == cAge3)
    {
        if ((kbGetTechStatus(cTechIrrigation) < cTechStatusResearching) && (aiPlanGetIDByTypeAndVariableType(cPlanProgression, cProgressionPlanGoalTechID, cTechIrrigation, true) >= 0))
        {
            ...
            {
                aiTaskUnitResearch(findUnit(Dropsite), cTechIrrigation);
                if (ShowAiEcho == true) aiEcho("Flushing military wood and gold escrow");
            }
        }
		...
		(aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, cUnitTypeSettlementLevel1, true) > 0))
        {
            if ((cMyCulture != cCultureEgyptian) && (woodSupply > 350) && (goldSupply > 350) && (foodSupply > 200) || (cMyCulture == cCultureEgyptian) && (goldSupply > 400) && (foodSupply > 250))
            {
                kbEscrowFlush(cEconomyEscrowID, cResourceWood, true);
                kbEscrowFlush(cMilitaryEscrowID, cResourceWood, true);
                kbEscrowFlush(cEconomyEscrowID, cResourceGold, true);
    ...
    else if (kbGetAge() > cAge3)
    {
        ...
        if ((kbGetTechStatus(cTechFloodControl) < cTechStatusResearching) && (aiPlanGetIDByTypeAndVariableType(cPlanProgression, cProgressionPlanGoalTechID, cTechFloodControl, true) >= 0))
        {
            ...
            {
                aiTaskUnitResearch(findUnit(Dropsite), cTechFloodControl);
            }
        }
        ...
        if ((aiGoalGetNumber(cGoalPlanGoalTypeBuildSettlement, cPlanStateWorking, true) > 0) && (kbUnitCount(0, cUnitTypeAbstractSettlement) > 0))
        {
            if ((cMyCulture != cCultureEgyptian) && (woodSupply > 350) && (goldSupply > 350) || (cMyCulture == cCultureEgyptian) && (goldSupply > 450) && (foodSupply > 250))
            {
                kbEscrowFlush(cEconomyEscrowID, cResourceWood, true);
                kbEscrowFlush(cMilitaryEscrowID, cResourceWood, true);
                kbEscrowFlush(cEconomyEscrowID, cResourceGold, true);
...
rule updateEMAge1       // i.e. cAge1
    minInterval 13 //starts in cAge1
    active
{
    ...
    inactive
{
	...
	float goldEscrow = econEscrow;
		

    //More military in second age
    updateEM(civPopTarget, milPopTarget, econPercent, 0.2, foodEscrow, woodEscrow, goldEscrow, econEscrow);
}
...
void updateGathererRatios(void) //Check the forecast variables, check inventory, set assignments
{
	...
	if (kbGetAge() > cAge3)
	numTradeCarts = numTradeCarts * 1.25;
...
{	
    ...
    if ((ageStartTime != -1) && (xsGetTime() - ageStartTime > 5*60*1000))
    {
        ...
        if (woodSupply < 800)
            gWoodForecast = gWoodForecast + (800 - woodSupply);
        if (goldSupply < 1100)
    ...
    // Fortress, etc.
    if (kbUnitCount(cMyID, cUnitTypeAbstractFortress, cUnitStateAliveOrBuilding) < 2)
	...
	// Villagers & Farms   
	temp = aiPlanGetVariableInt(gCivPopPlanID, cTrainPlanNumberToMaintain, 0);  
    ...
    inactive
{
    ...
    if ((ageStartTime != -1) && (xsGetTime() - ageStartTime > 4.5*60*1000))
    {
   ...
   if (cMyCulture == cCultureAtlantean)
    ...
    // military buildings
    if (kbUnitCount(cMyID, cUnitTypeLogicalTypeBuildingsThatTrainMilitary, cUnitStateAliveOrBuilding) < 3)
	{
	if (cMyCulture == cCultureEgyptian)
    ...
    inactive
{
    ...
    float woodSupply = kbResourceGet(cResourceWood);
    if ((gFarming == true) && (cMyCulture == cCultureEgyptian) && (kbUnitCount(cMyID, cUnitTypeFarm, cUnitStateAliveOrBuilding) < 10))
        addUnitForecast(cUnitTypeFarm, 3); 
...
void init(void)
{
    ...
    if (aiGetWorldDifficulty() == cDifficultyEasy)
        ...
        aiSetAttackResponseDistance(35.0);
    else
    ...
    if (rushSize < 25)
        rushSize = 25;  // Give unitpicker something to do...


    gRushUPID=initUnitPicker("Rush", numTypes, -1, -1, rushSize, rushSize*1.25, 2, false); // Rush with rushSize pop slots of two types, 2 buildings
    ...
    if (gRushUPID >= 0)
    {
		...
		if (cMyCulture == cCultureGreek)
        {
		 ...
		 if (cMyCiv == cCivPoseidon)
		 kbUnitPickSetPreferenceFactor(gRushUPID, cUnitTypeHippikon, 0.5);	 
        }
        if (cMyCulture == cCultureEgyptian)
        {
    ...
    active
{
 xsSetRuleMinIntervalSelf(25);
   ...
   if (ShowAiEcho == true) aiEcho("findFish:");
	...
	kbUnitQuerySetSeeableOnly(unitQueryID, false);  // help a poor AI out, semi cheating.. but a player would always know anyway.
