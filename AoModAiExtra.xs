...
//By Retherichus
//I'm so happy that you made it this far! this is where you'll find some of the code I've added to the Ai... or should I say " Stolen " / " Tweaked" ? heh.
//Feel free to copy/borrow my stuff for your own projects if you like, though some kind of credit would be appreciated! uwu
//Oh.. and suggestions are very welcome too, just don't expect me to add them right away.. 
//I'm quite busy these days, so busy that it is considered as unhealthy. :(
...
void initRethlAge2(void)
{
	...
	switch(cMyCulture)
	{
		case cCultureGreek:
		{
			...
			aiPlanSetEscrowID(MedicMaintain, cRootEscrowID);		
			break;
		}   
		...
		{	 
			...
			{	
		        ...
		        aiPlanSetVariableInt(gHero1MaintainPlan, cTrainPlanNumberToMaintain, 0, 2);
			}
			break;
		}
		...
		{	
			...
			aiPlanSetEscrowID(Turma, cRootEscrowID);
	...
	if ((cRandomMapName == "highland") || (cRandomMapName == "nomad") || (cvMapSubType == NOMADMAP))
	{ 
        if (kbUnitCount(0, cUnitTypeFish, cUnitStateAny) > 0)
		{
		    gWaterMap=true;
		    xsEnableRule("fishing");
		}
		if ((cRandomMapName == "nomad") || (cvMapSubType == NOMADMAP))
		{
...
active
{
	...
	if (AutoDetectMap == true)
	{
		...
		int TranspAlive = kbUnitCount(cMyID, transport, cUnitStateAlive);
		vector mainBasePos = kbBaseGetLocation(cMyID, mainBaseID);
		if ((mainBaseID == -1) && (kbUnitCount(cMyID, cUnitTypeAbstractSettlement, cUnitStateAlive) < 1) && (numSettlements > 0) 
		|| (TranspAlive > 0) && (mainBaseID != -1))
		{
		    if (mainBaseID == -1)
			{	
			    ...
			    mainBasePos = kbUnitGetPosition(findUnit(cUnitTypeAbstractVillager));
			}
			...
			{	    
				if (mainBaseID == -1)
				{
					...
					aiRemoveResourceBreakdown(cResourceFood, cAIResourceSubTypeEasy, base);
			...
			if ((ShowAIDebug == false) && (mainBaseID == -1)) aiEcho("Map has been detected as a Nomad Map!");
			else if ((ShowAIDebug == false) && (mainBaseID != -1)) aiEcho("Map has been detected as a Migration Map!");
		}
    ...
    if (kbGetAge() > cAge2)
    {
        ...
        if (cMyCulture == cCultureEgyptian && kbGetTechStatus(cTechAge3Hathor) == cTechStatusActive)
		{
			...
			if (gTransportMap == true)
			{
			    ...
			    aiPlanSetEscrowID(hRoc, cRootEscrowID);
			}
	    }			
		...
		if (cMyCulture == cCultureAtlantean)
		{
	        kbUnitPickSetPreferenceFactor(gLateUPID, cUnitTypeJavelinCavalry, 0.1);
	        kbUnitPickSetPreferenceFactor(gLateUPID, cUnitTypeTridentSoldier, 0.7);
	    }
	
		if (cMyCulture == cCultureEgyptian)
		{
			xsEnableRule("rebuildSiegeCamp");
			xsEnableRule("AdjustUP");
			if (cMyCiv == cCivRa)
			{
...
active
{
    if (kbGetAge() > cAge3)
    {
	    ...
	    if (cMyCulture == cCultureEgyptian && kbGetTechStatus(cTechAge4Horus) == cTechStatusActive)
		{
	        ...
	        kbUnitPickSetPreferenceFactor(gLateUPID, cUnitTypeSpearman, 0.8);
		    kbUnitPickSetPreferenceFactor(gLateUPID, cUnitTypeAxeman, 0.8);
		}
        ...
        if (cMyCulture == cCultureEgyptian && kbGetTechStatus(cTechAge4Thoth) == cTechStatusActive)
		{
			...
			kbUnitPickSetPreferenceFactor(gLateUPID, cUnitTypeWarElephant, 0.8);
		}
		...
		if (cMyCulture == cCultureAtlantean)
		{
		    kbUnitPickSetPreferenceFactor(gLateUPID, cUnitTypeSwordsman, 0.7);
		    kbUnitPickSetPreferenceFactor(gLateUPID, cUnitTypeRoyalGuard, 0.9);
		}
		if (cMyCiv == cCivSet)
...
group HateScripts
{
	...
	{
		int unitID = findUnitByIndex(UnitType, i, cUnitStateAlive);
		if ((cMyCulture == cCultureAtlantean) && (kbUnitIsType(unitID, cUnitTypeVillagerAtlanteanHero) == true))
		continue;		
		vector unitLoc = kbUnitGetPosition(unitID);
		int enemyID = findClosestUnitTypeByLoc(cPlayerRelationEnemy, UnitToCounter, cUnitStateAlive, unitLoc, Range);
	    if (cMyCulture != cCultureEgyptian)
...
inactive
{
	...
	if (activeAttPlans > 0)
    {
        for (i = 0; < activeAttPlans)
        {
			...
			if (attackPlanID == TransportAttPlanID)	
			{
				...
				if ((numInPlan < 1) || (xsGetTime() > attackPlanStartTime + 10*60*1000) || (planState == cPlanStateNone) && (xsGetTime() > attackPlanStartTime + 3*60*1000) ||
	...
	aiPlanAddUnitType(TransportAttPlanID, cUnitTypeLogicalTypeLandMilitary, 0, 0, 1);
...
inactive
{
	...
	if ((TransAlive > 1) || (Docks < 1) || (kbEscrowGetAmount(cRootEscrowID, cResourceWood) < Needed))
	return;
	...
	if ((currentPop >= currentPopCap - 5) && (currentPopCap > 100) && (TransInProgress > 0))
	{
		...
		if (Success == false)
		{
			for (i = 0; < kbUnitCount(cMyID, cUnitTypeHumanSoldier, cUnitStateAlive))
			{
				UnitToKill = findUnitByIndex(cUnitTypeHumanSoldier, i, cUnitStateAlive, -1, cMyID);
				if (UnitToKill == -1)
				continue;
				aiTaskUnitDelete(UnitToKill);
	...
	else if ((currentPop >= currentPopCap - 5) && (currentPopCap > 100) && (TransInProgress < 1))
...
active
{
			...
			for (i = 0; < ActivePlans)
			{
				...
				if (NumVar == 2) 
				{
					...
					if ((SpecialNum == 150) && (xsGetTime() > TimeActive))
					{
						...
						//aiEcho("Destroyed plan, it was active for too long ID: "+aiPlanGetName(PlanID)+", it was overdue by seconds: "+Overdue+"");
					}
				}
			}
		}
	}
    //Farm Patcher & Idle Villagers	& Favour flusher
	...
	if (aiGetWorldDifficulty() >= cDifficultyModerate) // micro Villagers on >= Moderate.
	{
        //EjectVillagers();
	    //MicroVillagers();
	}
    if ((kbResourceGet(cResourceFavor) >= 80) && (kbEscrowGetAmount(cRootEscrowID, cResourceFavor) < 50))
	{
	    //aiEcho(" Flushing Favour ");
...
//==================================================================================
rule AdjustUP   // For fortresses etc
    minInterval 17
    inactive
{     
	...
	if (cMyCulture == cCultureNorse)
	{
	    ...
	    kbUnitPickSetPreferenceFactor(gLateUPID, cUnitTypeRaidingCavalry, 0.4);
	}
			
	if ((cMyCulture == cCultureEgyptian) && (kbGetAge() == cAge3))
	{
        kbUnitPickSetPreferenceFactor(gLateUPID, cUnitTypeSpearman, 0.4);
	    kbUnitPickSetPreferenceFactor(gLateUPID, cUnitTypeAxeman, 0.3);
...
rule VinLandMBChange
minInterval 11
inactive
{
	int TcsOnMain = -1;
	if (sVinlandMG != -1)
	TcsOnMain = NumUnitsOnAreaGroupByRel(true, sVinlandMG, cUnitTypeAbstractSettlement, cMyID);
    if (TcsOnMain > 0)
    {
       int TC = FindSaferTC(true, sVinlandMG); // ignores start tc once.
	   if (TC != -1)
	   {
            aiRemoveResourceBreakdown(cResourceFood, cAIResourceSubTypeFarm, gFarmBaseID);
            //Get new base ID.
            gFarmBaseID = kbUnitGetBaseID(TC);
	        ...
	        changeMainBase(TC);
            VinOkToChange = true; // allow to change to start island now, as a last resort..
	        xsDisableSelf(); 
	    }
	}  
}
