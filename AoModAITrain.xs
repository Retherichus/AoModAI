    ...
    inactive
{
	...
	{
	gMaxTradeCarts = OldgMaxTradeCarts;
	...
	if ((NumTcs < 1) || (numIdleTrade > 3)) // don't train caravans if you have no TC or if too many are idle.
    return;
    ...
    inactive
{
    ...
    if ((cMyCulture == cCultureAtlantean) && (puid == age2MythUnitID) && (puid != cUnitTypeFlyingMedic))
    {
        ...
        aiPlanSetVariableInt(planID, cTrainPlanFrequency, 0, 14);
    }
...
{        
    ...
    else if (cMyCulture == cCultureAtlantean)
    {
        ...
        unitType2 = cUnitTypeSwordsman;
        unitType3 = cUnitTypeMaceman;
        if (kbGetAge() >= cAge3)
    ...
    if (numUnitType1 < Max)
    {
        ...
        {
            if ((woodSupply > 200) && (goldSupply > 150))
                unitTypeToTrain = unitType1;
        }
    }
    if (numUnitType2 < MaxB)
    {
        if (unitType2BeingTrained == false)
        {

            if ((foodSupply > 180) && (goldSupply > 195))
                unitTypeToTrain = unitType2;
        }
    }
	
    if (numUnitType3 < MaxC)
    {
        if (unitType3BeingTrained == false)
        {
              if ((foodSupply > 205) && (goldSupply > 120))
